...
# Usage:
#   ./run_move_all_with_rate.sh /src/dir /dst/dir [--overwrite]
#
...
if [[ -z "${SRC}" || -z "${DST}" ]]; then
  echo "Usage: $0 <sourceDir> <targetDir> [--overwrite]" >&2
  exit 2
fi

REPORT_EVERY=30000
LOG_FILE="${HOME}/log-of-speeds.txt"

# Truncate log at start of run (remove if you prefer appending)
: > "${LOG_FILE}"
...
sudo java MoveFilesNioSingleThreadAll "${SRC}" "${DST}" ${OVERWRITE:+${OVERWRITE}} \
